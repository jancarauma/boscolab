  ...
  visitParametric_plot_query = (ctx: Parametric_plot_queryContext): ParametricRangeQueryStatement | ErrorStatement => {
    ...
    if (ctx.expr(0).children.length === 1 && ctx.expr(0).children[0] instanceof IdContext) {
      ...
      if (
        ...
        this.addParsingErrorMessage(
          "Internal error parsing parametric plot syntax, this is a bug. Report to suporte@boscolab.com.br"
        );
        return { type: "error" };
      }
    }
    ...
    if (ctx.expr(1).children.length === 1 && ctx.expr(1).children[0] instanceof IdContext) {
      ...
      if (
        ...
        this.addParsingErrorMessage(
          "Internal error parsing parametric plot assignment syntax, this is a bug. Report to suporte@boscolab.com.br"
        );
        return { type: "error" };
      }
    }
    ...
    if (!(xQueryResult.statement?.type === "query" &&
      ...
      this.addParsingErrorMessage('Internal error parsing parametric plot syntax, this is a bug. Report to suporte@boscolab.com.br');
      return {type: "error"};
    } else {
  ...
  visitNumber_with_units = (ctx: Number_with_unitsContext): string => {
    ...
    if (unitBlockData.unitsValid) {
      try{
        ...
        this.addParsingErrorMessage(`Error parsing '${bignumber(original_value)} ${unitBlockData.units}'. This is an error that indicates a possible bug, report to suporte@boscolab.com.br`)
      } 
    }
