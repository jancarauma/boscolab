...
export default {
  async fetch(request: Request, env: Env) {
    ...
    if (url.hostname !== "boscolab.vercel.app" && !url.port && 
        response.headers.get("Content-Type")?.includes("text/html")) {
      const canonicalUrl = new URL(url);
      canonicalUrl.hostname = "boscolab.vercel.app";
      response = new HTMLRewriter()
...
async function postSheet({ origin, requestHash, requestBody, requestIp, kv, d1, useD1 }:
  ...
  if (createNewDocument) {
    ...
    if (alreadyExists) {
      return new Response("Sheet id collision, save unsuccessful. Try to save your document again. If issue persists, contact support at suporte@boscolab.com.br", { status: 404 });
    }
